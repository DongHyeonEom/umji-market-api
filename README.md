...
도메인별 헥사고날 아키텍처와 클린 아키텍처를 적용함. Controller는 입력 adapter, application UseCase는 업무 흐름, JPA 구현은 출력 adapter임. 상세 규칙과 Mermaid 흐름도는 [docs/architecture.md](docs/architecture.md)를 참고.
...
로컬 프로필은 인증을 우회하는 `BYPASS` 설정을 사용함. 운영 환경 사용 금지. 기본/운영 인증 모드는 `REQUIRED`임.
...
상태 확인: `GET /actuator/health`. 로컬 설정과 키는 저장소에 커밋하지 않음.
