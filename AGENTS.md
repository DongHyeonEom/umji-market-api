...
- 마침표(`.`)로 문장을 끝낸 뒤에는 가독성을 위해 줄바꿈을 적용함.
- 작업 착수 전 `docs/todo.md`에서 관련 항목의 상태와 수용 기준을 확인함. 신규 기능은 기존 항목에 연결하거나 체크리스트에 먼저 추가함.
- 작업 완료 시 실제 코드와 필요한 검증이 끝난 항목만 `[x]`로 변경함. 미완료·부분 완료·검증되지 않은 항목은 `[ ]`로 유지하고, 범위 변경은 체크리스트와 수용 기준에 반영함.
- 기능 작업의 체크리스트 변경은 해당 기능 브랜치와 PR에 포함함. 문서 작업 및 일반 수정의 체크리스트 변경은 해당 작업 브랜치의 기존 커밋 규칙을 따름.
