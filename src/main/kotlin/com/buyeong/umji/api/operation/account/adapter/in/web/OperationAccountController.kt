...
class OperationAccountController(private val useCase: OperationAccountUseCase) {
    @PostMapping @PreAuthorize("@operationAuthorization.hasPermission(authentication, 'ADMIN_ACCOUNT_MANAGE')") @ResponseStatus(HttpStatus.CREATED)
    ...
    @GetMapping @PreAuthorize("@operationAuthorization.hasPermission(authentication, 'ADMIN_ACCOUNT_MANAGE')")
    ...
    @GetMapping("/{id}") @PreAuthorize("@operationAuthorization.hasPermission(authentication, 'ADMIN_ACCOUNT_MANAGE')") fun detail(@PathVariable id: UUID) = useCase.detail(id).toResponse()
    @PatchMapping("/{id}/status") @PreAuthorize("@operationAuthorization.hasPermission(authentication, 'ADMIN_ACCOUNT_MANAGE')") fun status(@PathVariable id: UUID, @Valid @RequestBody request: UpdateAccountStatusRequest) = useCase.status(id, request.status).toResponse()
    @PutMapping("/{id}/business-profile") @PreAuthorize("@operationAuthorization.hasPermission(authentication, 'ADMIN_ACCOUNT_MANAGE')") fun profile(@PathVariable id: UUID, @Valid @RequestBody request: BusinessProfileRequest) = useCase.profile(id, request.toData()).toResponse()
    @PostMapping("/{id}/consents") @PreAuthorize("@operationAuthorization.hasPermission(authentication, 'ADMIN_ACCOUNT_MANAGE')") fun consent(@PathVariable id: UUID, @Valid @RequestBody request: CreateConsentRequest) = useCase.consent(id, ConsentCommand(request.consentType, request.documentVersion, request.consentMethod, request.evidenceReference.clean())).toResponse()
    @PostMapping("/{id}/approve") @PreAuthorize("@operationAuthorization.hasPermission(authentication, 'ADMIN_ACCOUNT_MANAGE')") fun approve(@PathVariable id: UUID) = useCase.approve(id).toResponse()
