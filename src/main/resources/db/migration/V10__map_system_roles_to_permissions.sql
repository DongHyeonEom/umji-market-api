...
JOIN permission p ON
    ...
    OR (r.code = 'PRODUCT_MANAGER' AND p.code IN ('PRODUCT_READ', 'PRODUCT_WRITE'))
    OR (r.code = 'ORDER_MANAGER' AND p.code IN ('ORDER_READ', 'ORDER_WRITE'))
    OR (r.code = 'INVENTORY_MANAGER' AND p.code IN ('INVENTORY_READ', 'INVENTORY_WRITE'))
