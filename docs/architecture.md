...
서비스의 최상위 기준은 헥사고날 아키텍처(Ports and Adapters)와 클린 아키텍처임. 의존성은 바깥 adapter에서 안쪽 application·domain을 향함. 안쪽 계층은 Spring MVC, Spring Data JPA, DB 구현, HTTP DTO를 참조하지 않음.
...
실선은 런타임 요청·호출 흐름이고 점선은 구현 의존 방향임. Application은 출력 Port를 호출하며, adapter가 DB나 외부 API에 연결함.
...
모든 도메인에 불필요한 계층을 기계적으로 만들지 않음. Port는 실제 경계가 필요한 곳에 두고, 이름보다 의존 방향을 준수. JPA Entity와 Spring Data Repository는 `persistence/jpa/{aggregate}` 구현 내부에 배치.

현재 adapter/application 경계를 적용한 도메인은 인증, 카탈로그, 장바구니, 주문, 재고, 관리자 계정, 관리자 카탈로그임. account 사용자 API, 결제, 파일, 알림 기능은 아직 구현되지 않았음.
...
현재는 하나의 API와 MySQL을 사용함. 헥사고날 구조는 내부 코드의 변경·테스트 경계를 만들며, 그 자체가 MSA 분리를 뜻하지 않음. 독립 배포·확장 요구와 데이터 소유권이 분명해질 때 모듈/서비스 분리를 검토함. Flyway 현황은 [database.md](database.md), 관계도는 [database-erd.md](database-erd.md)를 기준으로 함.
