...
서비스의 최상위 기준은 헥사고날 아키텍처(Ports and Adapters)와 클린 아키텍처임.
의존성은 바깥 adapter에서 안쪽 application·domain을 향함.
안쪽 계층은 Spring MVC, Spring Data JPA, DB 구현, HTTP DTO를 참조하지 않음.
...
실선은 런타임 요청·호출 흐름이고 점선은 구현 의존 방향임.
Application은 출력 Port를 호출하며, adapter가 DB나 외부 API에 연결함.
...
로컬 `BYPASS`는 보안 검사를 생략하며 운영 환경에서 사용 금지.
권한 매트릭스는 [operation.md](services/operation.md)를 기준으로 함.

## DB와 서비스 분리

현재는 하나의 API와 MySQL을 사용함.
헥사고날 구조는 내부 코드의 변경·테스트 경계를 만들며, 그 자체가 MSA 분리를 뜻하지 않음.
독립 배포·확장 요구와 데이터 소유권이 분명해질 때 모듈/서비스 분리를 검토함.
Flyway 현황은 [database.md](database.md), 관계도는 [database-erd.md](database-erd.md)를 기준으로 함.
