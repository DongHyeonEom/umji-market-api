...
- 운영자가 `휴대폰 본인 인증 생략 가능` 값을 허용한 `ACTIVE` 계정은 휴대폰 번호만으로 로그인함
- 그 외 기존 계정과 신규 계정은 휴대폰 본인 인증 후 기존 정보 확인 또는 신규 정보 입력을 진행함
- Access Token은 짧게 유지하고, Refresh Token은 명시적 삭제·무효화 또는 계정 상태 변경 전까지 기기별로 영속 유지함
- 구현 시 신규 Flyway 마이그레이션으로 계정의 휴대폰 본인 인증 요구 값과 영속 Refresh Token 정책을 반영함
